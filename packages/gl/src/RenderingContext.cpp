...
GLuint RenderingContext::EnsureElementArrayBuffer(
...
GLuint RenderingContext::EnsureTexture(std::shared_ptr<Texture> texture) {
  ...
  GLint format =
    (png.header.colorType == png::COLOR_TYPE_RGB) ? GL_RGB : GL_RGBA;
...
GLenum Convert(BufferAccessor::ComponentType type) {
  switch (type) {
    ...
    default:
      Panic(std::string("unsupported type ") +
...
void RenderingContext::VertexAttribPointer(GLuint location,
                                           const BufferAccessor& accessor) {
  glBindBuffer(GL_ARRAY_BUFFER, this->EnsureArrayBuffer(accessor.view->buffer));
  glVertexAttribPointer(location,
                        Convert(accessor.type),
                        Convert(accessor.componentType),
                        false,
                        0,
                        (void*)(accessor.view->offset));
}

void RenderingContext::SetActiveTexture(GLuint location,
...
void RenderingContext::DrawElements(const BufferAccessor& accessor) {
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,
               this->EnsureElementArrayBuffer(accessor.view));
  glDrawElements(GL_TRIANGLES,
                 static_cast<GLsizei>(accessor.view->length / sizeof(uint16_t)),
                 Convert(accessor.componentType),
                 nullptr);
}
