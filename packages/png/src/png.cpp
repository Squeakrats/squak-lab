...
IHDR parseIHDR(ByteStream& stream) {
  ...
  Assert(chunk.colorType == COLOR_TYPE_RGB ||
           chunk.colorType == COLOR_TYPE_RGBA,
         "Invalid color type");
...
void avg_inverse(uint8_t* dest,
  ...
  for (size_t i = 0; i < pixelSize; i++) {
    ...
    dest[i] = value + static_cast<uint8_t>(std::floor(
                        static_cast<double>(leftValue + aboveValue) / 2.0));
  }
}
...
void Paeth_inverse(uint8_t* dest,
                   uint8_t* above,
                   uint8_t* left,
                   uint8_t* upperLeft,
                   size_t pixelSize) {
...
PNG parse(BufferView view) {
  ...
  while (true) {
    ...
    if (chunkType == CHUNK_TYPE_IDAT) {
      compressedChunks.push_back(
        std::make_pair(stream.data + stream.pos, chunkSize));
  ...
  for (auto& range : compressedChunks) {
    std::memcpy(
      compressedData.data + compressedIndex, range.first, range.second);
  ...
  for (size_t line = 0; line < header.height; line++) {
    uint8_t* lineStart =
      inflated.data + (header.width * pixelSize + 1ll) * line;
    ...
    switch (filter) {
      ...
      case 1: { // Sub
        for (size_t x = 1; x < header.width; x++) {
          add(lineStart + 1 + x * pixelSize,
              lineStart + 1 + (x - 1) * pixelSize,
              pixelSize);
        }
        break;
      }
      case 2: { // Up
        ...
        uint8_t* lineAbove =
          inflated.data + (header.width * pixelSize + 1ll) * (line - 1);
        for (size_t x = 0; x < header.width; x++) {
          add(lineStart + 1 + x * pixelSize,
              lineAbove + 1 + x * pixelSize,
              pixelSize);
        }
        break;
      }
      ...
      case 4: { // Paeth
        ...
        for (size_t x = 0; x < header.width; x++) {
          ...
          uint8_t* left = (x != 0) ? lineStart + 1 + (x - 1) * pixelSize : zero;
          uint8_t* upperLeft =
            (x != 0 && line != 0) ? lineAbove + 1 + (x - 1) * pixelSize : zero;
          Paeth_inverse(
            lineStart + 1 + x * pixelSize, above, left, upperLeft, pixelSize);
        }
        break;
      }
      default:
